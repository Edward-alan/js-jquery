 ...
 * @Last Modified time: 2018-11-21 09:37:22
 */
/**
 * @func 封装jquery
 */
...
(function(window) {
    ...
    jQuery.fn = jQuery.prototype = {
            ...
            append: function(childNode) {
                ...
                this.each(function(i, item) {
                    if (typeof childNode === 'string') {
                        childNode = that.parseDom(childNode);
                        childNode = Array.from(childNode);
                        item.appendChild(childNode.cloneNode(true));

                        childNode.forEach(function(v, x) {
                                item.appendChild(v.cloneNode(true));
                            })
            ...
            removeClass: function(cName) {
                ...
                cName.forEach(function(item) {
                    that.each(function(i, ite) {
                        ite.classList.remove(item)
                    });
                })
                return this;
            },
            /**
             * @func [如果含有某个类名如果有没有添加]
             */
            ...
            children: function(string) {
                ...
                var that = this;
                var el = jQuery();
                this.each(function(x, y) { //调用childre方法可能有多个
                    that = y;
                    while (that.children) {
                        that = [...that.children];
                        that.forEach(function(item) { //childre可能有多个
                            if (that.nodeType !== 9) {
                                el.push(item)
                            }
                        })
            ...
            animate: function(obj, duration, fnCallback) {
                ...
                var oChange = {}; //改变值的集合
                var flag = 0;
                for (var attr in obj) { //获取每一个指的对象
                    var target = obj[attr]; //获取目标值的样式
                    var begin = that.getCss(this[0], attr); //获取初始值的样式
                    var change = target - begin; //改变部分=目标样式-初始样式
                    if (change) { //如果有改变
                        oBegin[attr] = begin; //开始样式一次添加
                        oChange[attr] = change;
                        flag++;
                    }
                }
    ...
    /**
     * @func [实现对象拓展,实现后面对象覆盖前面对象]
     */
    jQuery.extend = function() {
        var target = arguments[0];
        for (var i = 1; i < arguments.length; i++) { //传递参数不止一个
            for (var key in arguments[i]) { //将传过来的参数继续遍历
                target[key] = arguments[i][key] //后面传过来的每一项覆盖前面的每一项内容
            }
        };
        return target;
    };
    jQuery.jsonp = function(options) {
        ...
        script.src = options.url + (/\?/.test(options.url) ? '&' : '?') + this.formsParams(options.data) + '&' + options.callback + 'jsonp' + callbackName;
